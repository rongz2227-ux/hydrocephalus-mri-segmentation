...
log_dir = os.path.expanduser("~/projects/Hydro_Seg_Project/experiments_large_lvdice_a03")
...
# ===== E2: 额外的 LV Dice loss =====
alpha_lv = 0.3  # 先从 0.3 开始
smooth = 1e-5

def binary_dice_loss_from_logits(logits, targets, smooth=1e-5):
    """
    logits:  [B, 1, D, H, W]
    targets: [B, 1, D, H, W] 或 [B, D, H, W]
    """
...
for epoch in range(max_epochs):
    ...
    for i, batch_data in enumerate(train_loader):
        ...
        # 额外任务：只关注 LV(label=4) 的二值 Dice
        lv_targets = (labels == 4).float()          # [B, 1, D, H, W] 或 [B, D, H, W]
        lv_logits = outputs[:, 4:5, ...]            # 第4类对应的logits，shape [B,1,D,H,W]
        lv_loss = binary_dice_loss_from_logits(lv_logits, lv_targets, smooth=smooth)

        # 总 loss
        total_loss = main_loss + alpha_lv * lv_loss
    ...
    epoch_loss /= step
    print(f"Epoch {epoch + 1}/{max_epochs}, total_loss: {epoch_loss:.4f}")
    ...
    if (epoch + 1) % val_interval == 0:
        ...
        with torch.no_grad():
            for val_data in val_loader:
                ...
                val_outputs = sliding_window_inference(val_inputs, roi_size, 4, model, overlap=0.25)
            ...
            if metric > best_metric:
                ...
                torch.save(model.state_dict(), os.path.join(log_dir, "best_metric_model_large_lvdice.pth"))
                print("   [SAVED] New Best Large Model!")
